...
{
    [TestFixture]
    public class TDPostGreSqlConnectorTests
    {
        private string _connectionString = "Host=127.0.0.1;Port=5433;Database=postgres;Username=postgres;Password=YourStrong!Passw0rd;";
        ...
        private readonly Mock<ILogger<TDPostGreSQLConnector>> _mockLogger;
        ...
        private readonly TDPostGreSQLConnector _connector;

        public TDPostGreSqlConnectorTests()
        {
            ...
            _mockLogger = new Mock<ILogger<TDPostGreSQLConnector>>();
            ...
            _connector = new TDPostGreSQLConnector(
                _mockLogger.Object,
        ...
        public void Setup()
        {
            if (_skipTests)
            {
                Assert.Ignore("Skipping PostgreSQL tests because NO_SQL_DB environment variable is set to true");
                return;
            }
        ...
        [Test]
        public void DeserializeConnector_ResolvesToPostGreSQLType()
        {
            if (_skipTests)
            {
                Assert.Ignore("Skipping PostgreSQL tests because NO_SQL_DB environment variable is set to true");
            ...
            string json = @"
{
    ...
    ""Connections"": {
        ""ExampleDBName"": {
            ...
            Assert.That(mapping.Connections.First().Value.ConnectorType, Is.EqualTo(ConnectorType.PostGreSQL));
        }
        ...
        public void TestConnectionAsync_ShouldThrowException_WhenConnectionFails()
        {
            ...
            // Arrange
            _configuration.ConnectionString = "Host=invalid;Port=9999;Database=invalid;Username=invalid;Password=invalid;Timeout=1;";
        ...
        public void AddMemoryVariable_ShouldAddParameter_ForDifferentDataTypes()
        {
            ...
            Assert.That(command.Parameters["@stringVar"].NpgsqlDbType, Is.EqualTo(NpgsqlDbType.Varchar));
            Assert.That(command.Parameters["@stringVar"].Value, Is.EqualTo("test"));

            Assert.That(command.Parameters["@int32Var"].NpgsqlDbType, Is.EqualTo(NpgsqlDbType.Integer));
            Assert.That(command.Parameters["@int32Var"].Value, Is.EqualTo(123));

            Assert.That(command.Parameters["@int64Var"].NpgsqlDbType, Is.EqualTo(NpgsqlDbType.Bigint));
            Assert.That(command.Parameters["@int64Var"].Value, Is.EqualTo(9223372036854775807L));

            Assert.That(command.Parameters["@doubleVar"].NpgsqlDbType, Is.EqualTo(NpgsqlDbType.Double));
            Assert.That(command.Parameters["@doubleVar"].Value, Is.EqualTo(123.45));

            Assert.That(command.Parameters["@datetimeVar"].NpgsqlDbType, Is.EqualTo(NpgsqlDbType.Timestamp));
            Assert.That(command.Parameters["@datetimeVar"].Value, Is.EqualTo(new DateTime(2023, 1, 1)));

            Assert.That(command.Parameters["@boolVar"].NpgsqlDbType, Is.EqualTo(NpgsqlDbType.Boolean));
