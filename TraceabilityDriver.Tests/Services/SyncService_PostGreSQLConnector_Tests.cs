...
{
    [TestFixture]
    public class SyncService_PostGreSQLConnector_Tests
    {
        private string _postgreSqlConnectionString = "Host=127.0.0.1;Port=5433;Database=TraceabilityDriverTestDB;Username=postgres;Password=YourStrong!Passw0rd;";
        ...
        public void OneTimeSetup()
        {
            ...
            if (_skipTests)
            {
                Assert.Ignore("Skipping PostgreSQL synchronization tests because NO_SQL_DB environment variable is set to true");
                return;
            }
        ...
        private void SetupTestDatabase()
        {
            ...
            using (var connection = new NpgsqlConnection(_postgreSqlConnectionString.Replace("Database=TraceabilityDriverTestDB;", "Database=postgres;")))
            {
                connection.Open();
                ...
                using (var cmd = connection.CreateCommand())
                {
                    ...
                    cmd.CommandText = @"
                        ...
                        WHERE pg_stat_activity.datname = 'TraceabilityDriverTestDB'
                ...
                using (var cmd = connection.CreateCommand())
                {
                    cmd.CommandText = "CREATE DATABASE \"TraceabilityDriverTestDB\";";
                    cmd.ExecuteNonQuery();
                }
            }

            // Create test table and insert sample data
            using (var connection = new NpgsqlConnection(_postgreSqlConnectionString))
            {
                connection.Open();
        ...
        public async Task SynchronizeAsync_WithLastIDMemoryVariable_ShouldSyncCorrectly()
        {
            ...
            // Setup connector to return real PostgreSQL connector
            var mockConnectorLogger = new Mock<ILogger<TDPostGreSQLConnector>>();
            var mockTableMappingService = new Mock<IEventsTableMappingService>();

            var connector = new TDPostGreSQLConnector(
                ...
                {
                    {
                        ...
                        new TDConnectorConfiguration
                        {
                            ...
                            ConnectionString = _postgreSqlConnectionString,
